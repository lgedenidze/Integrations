...
namespace Integrations.Services
{
    public class AccountsRepository : IAccountsRepository
    {
        ...
        public async Task<OpenAccountResult> OpenAccount(OpenAccountParams openAccountParams)
        {
            OpenAccountResult v_result = null;
            try
            {
                using (OracleConnection oracle_conn = new OracleConnection(connectionString))
                {
                    ...
                    using (OracleCommand cmd = oracle_conn.CreateCommand())
                    {
                        cmd.CommandText = "INTEGRATIONS.OPEN_ACCOUNT";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.BindByName = true;
                        if (_commandTimeout != null)
                            cmd.CommandTimeout = _commandTimeout.Value;
                        ...
                        cmd.Parameters.Add("return_value", OracleDbType.Int32).Direction = ParameterDirection.ReturnValue;
                        cmd.Parameters.Add("P_CLIENT_ID", OracleDbType.Int32).Value = openAccountParams.customerId;
                        cmd.Parameters.Add("P_CCY_LIST", OracleDbType.Varchar2).Value = String.Join(",", openAccountParams.currencies.Select(x => x.currency));
                        cmd.Parameters.Add("P_REQUEST_ID", OracleDbType.Varchar2).Value = openAccountParams.requestId;
                        cmd.Parameters.Add("P_RECORDSET", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                        ...
                        if (ora_reader.HasRows)
                        {
                            ...
                            while (await ora_reader.ReadAsync())
                            {
                                v_result.iban = await ora_reader.IsDBNullAsync("iban") ? "" : ora_reader.GetString("iban");
                                if (!v_success)
                                {
                                    v_result.error = new();
                                    v_result.error.errorCode = ora_reader.GetInt32("error_code");
        ...
        public async Task<AddCurrencyResult> AddCurrency(AddCurrencyParams addCurrencyParams)
        {
            AddCurrencyResult v_result = null;
            try
            {
                using (OracleConnection oracle_conn = new OracleConnection(connectionString))
                {
                    ...
                    using (OracleCommand cmd = oracle_conn.CreateCommand())
                    {
                        cmd.CommandText = "INTEGRATIONS.ADD_CURRENCY_TO_ACCOUNT";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.BindByName = true;
                        if (_commandTimeout != null)
                            cmd.CommandTimeout = _commandTimeout.Value;
                        ...
                        cmd.Parameters.Add("return_value", OracleDbType.Int32).Direction = ParameterDirection.ReturnValue;
                        cmd.Parameters.Add("P_IBAN", OracleDbType.Varchar2).Value = addCurrencyParams.iban;
                        cmd.Parameters.Add("P_CCY_LIST", OracleDbType.Varchar2).Value = String.Join(",", addCurrencyParams.currencies.Select(x => x.currency));
                        cmd.Parameters.Add("P_RECORDSET", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
        ...
        public async Task<AccountsForCrediting> GetAccountsForCrediting(string phoneNumber, string iban, string personalNo)
        {
            AccountsForCrediting v_accounts = null;
            ...
            using (OracleConnection oracle_conn = new OracleConnection(connectionString))
            {
                ...
                using (OracleCommand cmd = oracle_conn.CreateCommand())
                {
                    cmd.CommandText = "INTEGRATIONS.GET_CUST_ACCOUNTS_FOR_CREDIT";
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.BindByName = true;
                    if (_commandTimeout != null)
                        cmd.CommandTimeout = _commandTimeout.Value;
                    ...
                    cmd.Parameters.Add("P_IBAN", OracleDbType.Varchar2).Value = iban;
                    cmd.Parameters.Add("P_RECORDSET", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    ...
                    if (ora_reader.HasRows)
                    {
                        v_accounts = new AccountsForCrediting();
                        v_accounts.accounts = new();
                        while (await ora_reader.ReadAsync())
                        {
                            AccountForCrediting v_account = new AccountForCrediting();
                            v_account.id = ora_reader.GetString("id");
                            v_account.iban = ora_reader.GetString("iban");
                            v_account.currency = ora_reader.GetString("ccy");
                            v_account.phoneNumber = await ora_reader.IsDBNullAsync("phone_number") ? "" : ora_reader.GetString("phone_number");
                            v_account.personalNo = await ora_reader.IsDBNullAsync("personal_No") ? "" : ora_reader.GetString("personal_No"); 
                            v_account.customerId = ora_reader.GetInt32("customerNo");
                            v_account.customerName = await ora_reader.IsDBNullAsync("cust_name") ? "" : ora_reader.GetString("cust_name");
                            v_account.customerNameEn = await ora_reader.IsDBNullAsync("cust_name_en") ? "" : ora_reader.GetString("cust_name_en");
                            v_accounts.accounts.Add(v_account);
                        }
                    }
                    await ora_reader.CloseAsync();
